...
interface PersonApi {
    @GET("api/v1/geneology/person/{personId}")
    suspend fun getPerson(@Path("personId") personId: String): ApiResult<PersonDTO>

    @DELETE("api/v1/geneology/person/{personId}")
    suspend fun deletePerson(
        @Path("personId") personId: String,
    ): ApiResult<Unit>

    @PATCH("api/v1/geneology/person/{personId}")
    suspend fun updatePerson(
        @Path("personId") personId: String,
        @Body body: PersonRequestDTO,
    ): ApiResult<PersonDTO>

    @POST("api/v1/geneology/person/{personId}/relation")
    suspend fun addRelation(
        @Path("personId") personId: String,
        @Body body: AddRelativeRequestDTO,
    ): ApiResult<PersonDTO>

    @GET("api/v1/geneology/tree/{treeId}/person")
